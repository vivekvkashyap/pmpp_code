...
__global__ void convolution_tiled_2d_const_mem_kernel(float *I, float *P, int width, int height){
    int col = blockIdx.x * OUT_TILE_DIM + threadIdx.x - FILTER_RADIUS;
    int row = blockIdx.y * OUT_TILE_DIM + threadIdx.y - FILTER_RADIUS;

    __shared__ float I_s[IN_TILE_DIM][IN_TILE_DIM];
        ...
        if (tileCol>=0 && tileCol<OUT_TILE_DIM && tileRow>=0 && tileRow<OUT_TILE_DIM){
            ...
            for (int fRow=0; fRow<2*FILTER_RADIUS+1; fRow++){
                for (int fCol=0; fCol<2*FILTER_RADIUS+1; fCol++){
                    Pval += F[fRow][fCol] * I_s[tileRow+fRow][tileCol+fCol];
                }
            }
...
int main(){
    ...
    dim3 dimGrid(ceil_div(N, OUT_TILE_DIM), ceil_div(N, OUT_TILE_DIM));

    printf("Performing warmup runs...\n");
    for (int i = 0; i < 3; i++){
        convolution_2d_cpu(h_I, h_P_cpu, h_f, FILTER_RADIUS, N, N);
        convolution_tiled_2d_const_mem_kernel<<<dimGrid, dimBlock>>>(d_I, d_P, N, N);
