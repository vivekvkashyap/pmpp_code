...
__global__ void convolution_2d_const_mem_kernel(float *I, float *P, int r, int width, int height){
    ...
    if (outRow < height && outCol < width) {
        ...
        for (int fRow = 0; fRow < 2*r+1; fRow++){
            for (int fCol = 0; fCol < 2*r+1; fCol++){
                ...
                if (inRow >= 0 && inRow < height && inCol >= 0 && inCol < width){
                    Pval += F[fRow][fCol] * I[inRow * width + inCol];
                }
            }
        }
...
int main(){
    ...
    for (int i = 0; i < 3; i++){
        ...
        convolution_2d_const_mem_kernel<<<dimGrid, dimBlock>>>(d_I, d_P, FILTER_RADIUS, N, N);
